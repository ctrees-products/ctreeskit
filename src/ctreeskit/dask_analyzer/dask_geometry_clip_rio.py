...
def geometry_clip_rio(
    ...
    *,
    ...
    invert = (
        not invert
    )  # rioxarray clip convention -> rasterio geometry_mask convention
    if xdim not in obj.dims or ydim not in obj.dims:
        raise ValueError(
            f"Received {xdim=!r}, {ydim=!r} but obj.dims={tuple(obj.dims)}"
        )
    bbox = tuple(geometries.total_bounds)  # Ensure bbox is a tuple of length 4
    print(f"bbox: {bbox}")
    obj = ctreeskit.clip_ds_to_bbox(obj, bbox)
    box = obj.odc.geobox
